1. 功能说明
    此例程展示在 RT_Thread 静态互斥量和动态互斥量的创建、获取和释放
...
3. 使用说明
    描述相关模块配置方法；例如:时钟，I/O等 
         ...
         2. GPIO：日志:   TX--PA9  RX--PA10    波特率：115200

    描述Demo的测试步骤和现象 
         1. 编译后下载程序复位运行
         2. 接好串口打印工具
         3. 本例程在main()里面创建两个线程，thread1 线程和 thread2 线程，查看打印日志。
             thread2 先获取静态互斥量，等到 thread2 释放静态互斥量后 thread1 才能获取到静态互斥量，再释放静态互斥量；
