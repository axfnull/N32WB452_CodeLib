1. 功能说明
    此例程展示在 RT_Thread 消息队列的创建、发送和接收
...
3. 使用说明
    描述相关模块配置方法；例如:时钟，I/O等 
         ...
         2. GPIO：PD8（KEY04），PD9(KEY05）, PA0(KEYUP）
                        日志：TX--PA9  RX--PA10，波特率：115200

    描述Demo的测试步骤和现象 
         1. 编译后下载程序复位运行
         2. 接好串口打印工具
         3. 本例程创建两个线程，key 线程和 test 线程，key 线程检测按键发送消息队列，test 线程接收消息队列，按下按键（KEY04、KEY05、KEYUP）发送消息队列，查看打印日志
...
3. Instructions for use
    ...
    Describe the test steps and phenomena of Demo 
        ...
        3. This routine creates two threads, the key thread and the test thread. The key thread detects the key sending message queue, and the test thread receives the message queue. 
            Press the key (KEY04, KEY05, KEYUP) to send the message queue and view the print log
